...
log = get_logger(log_dir=".", job_name="train_holdout")
...
def train_holdout(agent, env, train_replay, eval_replay, logger, args):
    ...
    log.info(f"Observation space: {embodied.format(env.obs_space)}")
    log.info(f"Action space: {embodied.format(env.act_space)}")
    ...
    while len(eval_replay) < max(args.batch_steps, args.eval_fill):
        log.info(f"{len(eval_replay)} {max(args.batch_steps, args.eval_fill)}")
    ...
    while len(train_replay) < max(args.batch_steps, args.train_fill):
        log.info(f"{len(train_replay)} {max(args.batch_steps, args.train_fill)}")
