...
log = get_logger(log_dir=".", job_name="distr")
...
class Client:
    def __init__(self, address, timeout_ms=-1, ipv6=False):
        ...
        for address in addresses:
            log.info(f"Client connecting to {address}")
...
class Server:
    def __init__(self, address, function, ipv6=False):
        ...
        log.info(f"Server listening at {address}")
        ipv6 and self.socket.setsockopt(zmq.IPV6, 1)
...
class BatchServer:
    def __init__(self, address, batch, function, ipv6=False):
        ...
        log.info(f"BatchServer listening at {address}")
        ipv6 and self.socket.setsockopt(zmq.IPV6, 1)
        self.socket.bind(address)
        self.function = function
        self.batch = batch
...
class Thread(threading.Thread):
    ...
    def _wrapper(self, *args):
        try:
            ...
            with self.lock:
                log.error(f"Exception in worker: {self.name}")
    ...
    def terminate(self):
        ...
        log.info(f"Shut down worker: {self.name}")
...
def run(workers):
    ...
    while True:
        if all(x.exitcode == 0 for x in workers):
            log.info("All workers terminated successfully.")
            return
