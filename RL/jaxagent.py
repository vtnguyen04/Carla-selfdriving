...
log = get_logger(log_dir=".", job_name="jaxagent")
...
class JAXAgent(embodied.Agent):
    def __init__(self, agent_cls, obs_space, act_space, step, config):
        ...
        log.info(f"JAX devices ({jax.local_device_count()}): {available}")
        log.info(f"Policy devices: {', '.join([str(x) for x in self.policy_devices])}")
        log.info(f"Train devices: {', '.join([str(x) for x in self.train_devices])}")
    ...
    def _setup(self):
        try:
            ...
            log.error(f"Could not disable TensorFlow devices: {e}")
