...
log = get_logger(log_dir=".", job_name="train_eval")
...
def train_eval(agent, train_env, eval_env, train_replay, eval_replay, logger, args):
    ...
    log.info(f"Observation space: {embodied.format(train_env.obs_space)}")
    log.info(f"Action space: {embodied.format(train_env.act_space)}")
    ...
    while step < args.steps:
        if should_eval(step):
            log.info(f"Starting evaluation at step {int(step)}")
            driver_eval.reset()
